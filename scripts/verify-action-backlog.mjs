...
const allFlows = readdirSync(flowDir).filter((file) => /^20\d{6}\.json$/.test(file)).sort().map((file) => JSON.parse(readFileSync(new URL(file, flowDir), 'utf8')));
const flows = allFlows.filter((flow) => flow.date >= ledger.auditedFrom && flow.date <= ledger.auditedThrough);
